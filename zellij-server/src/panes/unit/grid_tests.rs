...
fn sixel_replacement_reaps_covered_images_without_reusing_live_ids() {
    ...
    for step in 0..18 {
        ...
        if step >= 12 {
            // Yazi erases with spaces before drawing a differently sized preview.
            parser.advance(&mut grid, b"\x1b[3;4H   ");
        }
        // Repaint away from the origin, as a file-manager preview does.
        let bytes = format!(
    ...
    // An untouched opaque blank raster still paints the terminal background.
